        ...
        <div className="menuItems border-2 border-black rounded-[3px] mt-[0.7rem] mx-auto mb-[0.1rem] p-[0.3rem]">
          {
            menuItems?.map((items, index) => {
              return (index !== 0 && index !== 7 && index !== 8 && items?.card?.card?.itemCards?.length > 0) ? (
                <div className="categorizedItems border-2 border-black rounded-[3px] mt-[0.4rem] mx-auto mb-0 p-[0.3rem]" key={index}>
                  <div className="categoryTitle border-2 border-black rounded-[3px] text-center text-[25px] font-bold my-2 mx-auto p-[0.3rem]">{items?.card?.card?.title}</div>
                  <div className="categoryItems">
                    {
                      items?.card?.card?.itemCards?.map((item) => {
                        const { name, description, imageId, isVeg, price, id, variantsV2 } = item.card.info;
                        return (
                          <div className="foodItem border-2 border-black rounded-[3px] mt-2 mx-auto mb-[0.1rem] p-[0.3rem] flex items-start justify-between" key={id}>
                            <div className="foodDetails w-[80%] p-[0.1rem] my-2 mx-auto">
                            ...
                            <div className="img_add">
                              ...
                              <button className="add text-center block my-4 mx-auto cursor-pointer font-bold py-[0.3rem] px-2 text-sm w-[100%] rounded-[3px] tracking-[0.1rem] transition-all duration-100 ease-in-out border-black border-2 hover:uppercase">Add</button>
                              {/* <div className="added flex items-center justify-between my-4 mx-auto cursor-pointer font-bold p-[0.3rem] text-base rounded-[3px] border-black border-2 transition-all duration-100 ease-in-out hover:bg-black hover:text-white">
                                ...
                                <button className="plus tracking-[0.1rem] bg-transparent py-0 px-1">+</button>
                              </div> */}
                            </div>
                          </div>
                        );
